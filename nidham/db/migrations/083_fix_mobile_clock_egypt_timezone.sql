-- ============================================================================
-- Migration 083 — Anchor mobile clock-in/out on Egypt local time
-- ============================================================================
-- mobile_clock_in / mobile_clock_out (last defined in migration 020) used
-- `current_date` to decide which day a punch belongs to. Postgres/Supabase
-- runs in UTC, so `current_date` only flips to the next day at 02:00 Cairo
-- time (UTC+2, no DST). Consequences:
--   • A punch between 00:00 and 02:00 Cairo is filed on YESTERDAY's date.
--   • Night-shift workers clocking in after 02:00 land on the wrong day, and
--     mobile_clock_out then can't find "today's" row → "لازم تثبت حضور الأول".
--
-- Fix: compute the business day as (now() AT TIME ZONE 'Africa/Cairo')::date
-- in both functions. Hours-worked math is unaffected (it subtracts two
-- timestamptz values, which are absolute). Bodies are otherwise identical to
-- migration 020.
-- ============================================================================
...
declare
  ...
  v_distance          numeric;
  v_outside           boolean;
  -- Same Egypt-local business day as mobile_clock_in, so clock-out finds
  -- the row that clock-in created instead of looking under the UTC date.
  v_today             date := (now() at time zone 'Africa/Cairo')::date;
  v_row_id            uuid;
