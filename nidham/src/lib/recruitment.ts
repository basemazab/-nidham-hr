...
// ============================================================================
// Job Description generator — a real, company-tailored توصيف وظيفي for any
// role, printable to hand to the employee (NOT a generic template).
// ============================================================================

export const jobDescriptionSchema = z.object({
  job_title: z.string().describe("المسمى الوظيفي"),
  job_purpose: z
    .string()
    .min(10)
    .describe("الغرض من الوظيفة — 2-3 جمل محددة لنشاط الشركة (مش عامة)"),
  responsibilities: z
    .array(z.string())
    .min(6)
    .max(14)
    .describe("المهام والمسؤوليات اليومية — محددة ومرتبطة بنشاط الشركة فعلاً"),
  qualifications: z
    .array(z.string())
    .min(3)
    .max(10)
    .describe("المؤهلات والمهارات والخبرات المطلوبة"),
  reports_to: z.string().describe("يتبع (المسمى الأعلى المقترح)"),
  kpis: z
    .array(z.string())
    .min(3)
    .max(8)
    .describe("مؤشرات قياس أداء الوظيفة (KPIs) — قابلة للقياس"),
  industry_note: z
    .string()
    .describe("جملة توضح إزاي خصّصت التوصيف لنشاط الشركة تحديدًا"),
});
...
export function buildJobDescriptionPrompt(
  ...
  return `أنت خبير موارد بشرية مصري. مهمتك كتابة **توصيف وظيفي رسمي وحقيقي** لوظيفة محددة في شركة محددة — يُطبع ويُسلَّم للموظف. ممنوع الكلام العام؛ لازم المهام والمسؤوليات تكون مرتبطة بنشاط الشركة ده بالظبط. اكتب بعربي فصيح واضح ومهني.

═══════════════════════════════════════
الشركة (افهم طبيعتها كويس)
═══════════════════════════════════════
اسم الشركة: ${company.name ?? "غير محدد"}
النشاط/المجال: ${company.industry ?? "غير محدد"}

استنتج طبيعة عمل الشركة من اسمها ومجالها، واجعل المهام مرتبطة بها فعلاً (مثلًا: مصنع أبواب → مهام إنتاج وجودة وصيانة خطوط؛ شركة مقاولات → متابعة مواقع وحصر كميات وسلامة؛ شركة برمجيات → تطوير واختبار ونشر؛ عيادة → استقبال مرضى وملفات طبية؛ شركة تجارية → بيع وتحصيل وعلاقات عملاء).

═══════════════════════════════════════
الوظيفة المطلوب توصيفها
═══════════════════════════════════════
المسمى: ${jobTitle}
...
المطلوب
═══════════════════════════════════════
- job_purpose: الغرض من الوظيفة في سياق نشاط الشركة (2-3 جمل).
- responsibilities: 8-12 مهمة ومسؤولية **محددة لنشاط الشركة** (مش عبارات عامة زي "أداء المهام الموكلة").
- qualifications: المؤهلات والمهارات والخبرة المطلوبة للدور ده.
- reports_to: المسمى الأعلى اللي بيتبعه منطقياً.
- kpis: 3-6 مؤشرات أداء **قابلة للقياس** مناسبة للدور ونشاط الشركة.
- industry_note: جملة توضح إزاي خصّصت التوصيف لنشاط الشركة.
