...
- أو **استيراد من بصمة ZKTeco** لو عندك جهاز`,
...

  {
    slug: "zkteco-import",
    category: "attendance",
    title: "ربط جهاز البصمة (ZKTeco) — لحظي أو استيراد ملف",
    excerpt: "اربط جهاز ZKTeco يبعت البصمات لحظيًا للنظام، أو ارفع ملف للأجهزة القديمة.",
    body: `في نِظام فيه طريقتين لربط جهاز البصمة — الأفضل هي الربط اللحظي.

## الطريقة 1 (الموصى بها): ربط لحظي — الجهاز يبعت لوحده (Cloud-Push / ADMS)
أجهزة ZKTeco اللي بتدعم **Cloud Server Setup** بتبعت كل بصمة للنظام لحظيًا أول ما الموظف يبصم — تظهر تلقائي في صفحة مراجعة الحضور، بدون أي تصدير يدوي.

**خطوة 1 — سجّل الجهاز في النظام:**
**Dashboard ← الإعدادات ← أجهزة البصمة** ← أضف الجهاز بالاسم و**الرقم التسلسلي (Serial)** الظاهر في قائمة الجهاز.

**خطوة 2 — ظبط الجهاز يبعت للنظام:** في إعدادات الشبكة/السحابة بالجهاز:
- **Server Address:** \`www.nidhamhr.com\`
- **Port:** \`443\` — وفعّل **HTTPS / Domain** لو متاح

بعد كده الجهاز هيبدأ يبعت البصمات لوحده. النظام بيطابق **كود البصمة** بالموظف، ويطوي بصمات اليوم في صف واحد (أول بصمة = حضور، آخر بصمة = انصراف).

> ⚠️ قيد مهم: سيرفر النظام **HTTPS فقط (Port 443)**. الأجهزة القديمة اللي بتدعم HTTP بس مش هتعرف توصل مباشرة — استخدم معاها الطريقة 2.

## الطريقة 2 (للأجهزة القديمة): استيراد ملف
الموظف بيبصم والجهاز يحفظ السجلات؛ والـ HR ترفع ملف كل فترة:

**Dashboard ← الحضور ← استيراد** ← tab **"بصمة"** ← ارفع ملف الجهاز (XLS/TXT/ATTLOG، وكمان PDF بالـ AI) ← النظام يعرض preview (عدد السجلات + الموظفين الـ matched والـ unmatched) ← أكّد الحفظ.

## كود الموظف في الجهاز ≠ في Nidham؟ (مهم للطريقتين)
كل موظف عنده **"كود البصمة"** في الـ profile — لازم يطابق الكود الموجود في الجهاز بالظبط، وإلا البصمة تطلع unmatched.

**Dashboard ← الموظفين ← اختر الموظف ← كود الموظف (لربط البصمة)**

## أجهزة مدعومة
- **ZKTeco** — ربط لحظي للموديلات اللي بتدعم Cloud Server؛ واستيراد ملف لأي موديل بيدّي XLS export
- TimeWell
