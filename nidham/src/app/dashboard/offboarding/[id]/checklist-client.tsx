...
import { toggleChecklistItem, getChecklistProgress } from "../../onboarding/actions";
...
const CATEGORIES: ChecklistCategory[] = [
  {
    ...
    icon: "📋",
    items: [
      { key: "exit_interview", label: "مقابلة نهاية الخدمة" },
      { key: "final_settlement", label: "تجهيز التسوية النهائية" },
      { key: "experience_certificate", label: "شهادة خبرة" },
    ],
  },
  {
    key: "it",
    ...
    icon: "💻",
    items: [
      { key: "email_revoked", label: "إلغاء البريد الإلكتروني" },
      { key: "access_revoked", label: "إلغاء صلاحيات الأنظمة" },
      { key: "laptop_returned", label: "استلام اللاب توب" },
      { key: "data_backed_up", label: "نسخ احتياطي للبيانات" },
    ],
  },
  {
    key: "facilities",
    ...
    icon: "🏢",
    items: [
      { key: "keys_returned", label: "استلام المفاتيح / بطاقة الدخول" },
      { key: "desk_cleared", label: "تسليم المكتب" },
    ],
  },
  {
    key: "finance",
    ...
    icon: "💰",
    items: [
      { key: "dues_calculated", label: "احتساب المستحقات النهائية" },
      { key: "last_salary_processed", label: "صرف آخر راتب" },
    ],
  },
];

const STORAGE_KEY = (employeeId: string) => `offboarding_${employeeId}`;
...
export function OffboardingChecklistClient({
  ...
  useEffect(() => {
    ...
    getChecklistProgress(employeeId, "offboarding").then((res) => {
  ...
  const handleToggle = useCallback(
    async (itemKey: string) => {
      ...
      setChecked(updated);
      persist(updated);
      await toggleChecklistItem(employeeId, itemKey, next, "offboarding");
    },
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-6 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <div className="font-bold font-cairo text-slate-800">
            تقدم إجراءات التسكين
          </div>
          <div className="text-sm font-black font-cairo text-rose-600">
            {done}/{total} — {progress}%
          </div>
        </div>
        ...
        {loaded && progress === 100 && (
          <div className="mt-3 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-bold font-cairo text-center">
            ✅ تم استكمال جميع إجراءات إنهاء الخدمة
          </div>
        )}
      </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-3 rounded-xl border text-right transition ${
                        isChecked
                          ...
                          : "bg-slate-50 border-slate-100 hover:border-rose-300"
                      }`}
                    >
                      <div
