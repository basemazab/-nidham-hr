...
export async function POST(req: Request) {
  ...
  if (!cvText && file) {
    ...
    if (isText) {
      ...
      // CVs are quality-critical + user-facing. The byte-level PDF parser
      // mangles many real CVs (subsetted fonts / positioned glyphs come out as
      // letter-salad like "HME D M OSH IER PER", and the AI then fills every
      // field with "[غير واضح]"). So for the translator we ALWAYS read the file
      // with Gemini multimodal — it reads text AND scanned PDFs accurately.
      // Volume here is low (a deliberate, rate-limited action), so the Gemini
      // cost is negligible next to the quality win.
      {
  ...
  // Don't translate garbage: if the extracted text is corrupt salad (broken
  // PDF text layer), tell the user the exact fix instead of producing a fake
  // "[غير واضح]" translation.
  if (looksLikeCorruptText(cvText)) {
    return Response.json(
      {
        error:
          "الملف ده طبقة النص الداخلية فيه تالفة — بيظهر صح بس النص اللي جواه مكسور، فمش هينفع نترجمه صح. الحل الأسرع: ارفع الـCV كصورة (لقطة شاشة أو صورة واضحة بالموبايل) أو الصق نصه مباشرة — وهيتقرا تمام.",
      },
      { status: 422 },
    );
  }
